...
IGpioPin* STM32F207IGSystemGpioPort::getPin(uint8 uiNr)
{
  ...
  {
    aPins[uiNr] = CCNEW_INLINE(STM32F207IGSystemGpioPin, pPort, uiNr);
  }
  return aPins[uiNr];
}
...
bool STM32F207IGSystemGpioPort::setPinValue(size_t uiPin, bool bValue)
{
  bool bRet = false;
  IGpioPin* pPin = getPin(uiPin);
  if(pPin)
...
bool STM32F207IGSystemGpioPort::getPinValue(size_t uiPin)
{
  bool bRet = false;
  IGpioPin* pPin = getPin(uiPin);
  if(pPin)
