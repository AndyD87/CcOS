...
CcFilePointer CcFilesystemDevice::getFile(const CcString& ) const
{
  CcFilePointer file;
...
CcStatus CcFilesystemDevice::mkdir(const CcString &Path) const
{
...
CcStatus CcFilesystemDevice::remove(const CcString &Path) const
{
  CcStatus oResult(false);
  CCUNUSED(Path);
  return oResult;
