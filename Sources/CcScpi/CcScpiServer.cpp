 ...
 * @par       Language: C++11
 * @brief     Implementation of Class CcScpiServer
...
CcScpiServer::CcScpiServer()
{
}

CcScpiServer::CcScpiServer(const CcScpiServerConfig& oConfig) :
  m_oConfig(oConfig)
{
}
...
void CcScpiServer::run()
{
  ...
  if (m_Socket.bind(m_oConfig.getPort()))
  {
    while (getThreadState() == EThreadState::Running)
    {
      ...
      {
        CCDEBUG("CcScpiServer: incomming connection.");
        CCNEWTYPE(worker, CcScpiServerWorker, oReceived, oNewSocket, &m_oConfig);
        worker->start();
      }
      else
      {
        CCDEBUG("CcScpiServer::run Client Bind failed");
      }
    }
  }
  else
  {
    CCDEBUG("CcScpiServer::run Bind failed");
  }
}
