 ...
 * @brief     Implementation of Class LinuxGPIOPin
 */
...
LinuxGPIOPin::LinuxGPIOPin(uint8 nr) :
  m_uiPinNr(nr)
{
...
void LinuxGPIOPin::init( void )
{

}

bool LinuxGPIOPin::setDirection( EDirection eDirection)
{
  bool bRet =false;
  switch(eDirection)
  {
...
CcGPIOPin::EDirection LinuxGPIOPin::getDirection(void)
{
...
void LinuxGPIOPin::setValue(bool bValue)
{
  CcFile cFile(m_sPinPath + c_sGpioAppendValue);
  if(cFile.exists())
  {
...
bool LinuxGPIOPin::getValue(void)
{
  bool bRet = false;
  CcFile cFile(m_sPinPath + c_sGpioAppendValue);
  if(cFile.exists())
