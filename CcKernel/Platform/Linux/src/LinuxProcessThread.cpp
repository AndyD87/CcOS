 ...
 * @brief     Implemtation of class LinuxProcessThread
 */
...
LinuxProcessThread::LinuxProcessThread(CcProcess& rProcess) :
  m_hProcess(&rProcess)
{
}

LinuxProcessThread::~LinuxProcessThread( void )
{
}

void LinuxProcessThread::run()
{
  pid_t uiResult = fork();
  if(uiResult == 0)
  {
    dup2(static_cast<LinuxPipe&>(m_hProcess->pipe()).getPipes()[1], STDOUT_FILENO);
    close(static_cast<LinuxPipe&>(m_hProcess->pipe()).getPipes()[0]);
    close(static_cast<LinuxPipe&>(m_hProcess->pipe()).getPipes()[1]);
  ...
  else if(uiResult > 0)
  {
    ...
    close(static_cast<LinuxPipe&>(m_hProcess->pipe()).getPipes()[0]);
    close(static_cast<LinuxPipe&>(m_hProcess->pipe()).getPipes()[1]);
    static_cast<LinuxPipe&>(m_hProcess->pipe()).getPipes()[0] = -1;
    static_cast<LinuxPipe&>(m_hProcess->pipe()).getPipes()[1] = -1;
