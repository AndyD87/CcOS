 ...
 * @par       Language: C++11
 * @brief     Implementation of Class CcWindowsRegistryFilesystem
 */
...
CcStatus CcWindowsRegistryFilesystem::mkdir(const CcString& Path) const
{
  CCUNUSED(Path);
  return false;
}

CcStatus CcWindowsRegistryFilesystem::remove(const CcString& Path) const
{
  CCUNUSED(Path);
  return false;
}
