 ...
 * @brief     Implementation of Class WindowsPipe
 */
...
size_t WindowsPipe::read(char* buffer, size_t size)
{
  DWORD uiWriteSize;
...
size_t WindowsPipe::write(const char* buffer, size_t size)
{
  DWORD readSize=0;
  if (readSize > 0)
  {
...
bool WindowsPipe::open(EOpenFlags)
{
  return true;
}

bool WindowsPipe::close()
{
  return true;
}

bool WindowsPipe::cancel()
{
  return true;
}